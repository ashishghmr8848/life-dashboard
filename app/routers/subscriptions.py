...
def list_subscriptions(active_only: bool = False, db: Session = Depends(get_db)):
...
def get_subscription(subscription_id: uuid.UUID, db: Session = Depends(get_db)):
...
def update_subscription(subscription_id: uuid.UUID, payload: SubscriptionUpdate, db: Session = Depends(get_db)):
    subscription = db.query(Subscription).filter(Subscription.id == subscription_id).first()
    if not subscription:
        raise HTTPException(status_code=404, detail="Subscription not found")
